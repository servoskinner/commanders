...
struct Vector2i
{
    ...
    /// @brief taxicab metric length.
    constexpr inline int len_taxi() { return abs(x) + abs(y); }
    /// @brief chebyshev metric length.
    constexpr inline int len_inf() { return std::max(abs(x), abs(y)); }
