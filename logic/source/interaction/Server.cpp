...
//         std::optional<std::reference_wrapper<Client_slot>> sender;
//         auto sender_iter = std::find_if(client_slots.begin(), client_slots.end(), \
//                                         [&inbound](std::optional<Client_slot>>& c_info) \
//                                         {return c_info.has_value() && c_info->get().sock_info == inbound.sender;});
...
//                     if (sender.has_value())
//                     {
//                         sender->get().upkeep = CLIENT_UPKEEP_COUNTDOWN;
//                     }
//                 }
//                 break;
...
//         case MSG_GAMEPLAY:
//             {
//                 if(sender.has_value() && sender->get().commander.has_value())
//                 {
//                     std::vector<char> order_packed = inbound.msg;
//                     order_packed.erase(order_packed.begin());
//                     sender->get().commander->get().queue_order({order_packed});
//                 }
//             }
//             break;
